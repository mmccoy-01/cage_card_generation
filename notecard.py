...
HEADER_NAMES = {
    ...
    "cage_tag": ["cage tag"],
    "cage_barcode": ["cage barcode", "cage barcode value"],
...
def excel_display_value(excel_cell: Any) -> Any:
    """Return a cell value while preserving zero-padded Excel numbers.

    Excel may store a barcode displayed as ``001234`` as the numeric value
    ``1234`` with a number format such as ``000000``. Reading only
    ``cell.value`` drops those leading zeroes. This helper reproduces that
    displayed zero padding. Barcodes stored as text are returned unchanged.
    """
    ...
    if isinstance(value, (int, float)):
        number_format = safe_str(getattr(excel_cell, "number_format", ""))
        # Excel formats can contain separate positive/negative/zero/text
        # sections. Barcode formats are normally a single run of zeroes.
        positive_format = number_format.split(";", 1)[0].strip()
        if re.fullmatch(r"0+", positive_format):
            try:
                numeric_value = int(value)
                if float(value) == float(numeric_value):
                    return f"{numeric_value:0{len(positive_format)}d}"
...
def _rows_from_source(source: str | Path | bytes | BinaryIO | pd.DataFrame) -> tuple[list[list[Any]], list[str]]:
    ...
    ws = wb.active
    excel_rows = list(ws.iter_rows(values_only=False))
    if not excel_rows:
        return [], captured_warnings

    barcode_header_names = {normalize_header(name) for name in HEADER_NAMES["cage_barcode"]}
    barcode_columns = {
        index
        for index, excel_cell in enumerate(excel_rows[0])
        if normalize_header(excel_cell.value) in barcode_header_names
    }

    rows: list[list[Any]] = []
    for row_index, excel_row in enumerate(excel_rows):
        rows.append(
            [
                excel_display_value(excel_cell)
                if row_index > 0 and column_index in barcode_columns
                else excel_cell.value
                for column_index, excel_cell in enumerate(excel_row)
            ]
        )
    return rows, captured_warnings
...
def load_cages(
    ...
    for raw in data_rows:
        ...
        cage_barcode = safe_str(cell(raw, header_index, "cage_barcode"))
...
def draw_front_card(
    ...
    cage_number = safe_str(cage.get("cage_tag", ""))
    cage_barcode = safe_str(cage.get("cage_barcode", ""))
    cage_header = f"Cage #: {cage_number}"
    if cage_barcode:
        cage_header += f"\nCage Barcode: {cage_barcode}"
    draw_span_cell(
        c,
        cols,
        widths,
        ...
        cage_header,
        bold=True,
        size=6.8 if cage_barcode else 8.6,
        align="center",
        fill=HEADER_FILL,
        wrap=bool(cage_barcode),
        pad=0.7 * mm if cage_barcode else 1.3 * mm,
    )
    y -= h
...
def build_notecards_pdf_bytes(
    ...
    if not cages:
        raise ValueError(
            "No cage cards were generated. Confirm that the workbook contains cage rows "
            "and that at least one row is selected in the Print Card? column."
        )
    front_pages = math.ceil(len(cages) / CARDS_PER_PAGE)
