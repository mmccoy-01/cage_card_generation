Pasted code(1).py
Python
can you fix the code below so it also includes Cage Barcode value on the cage card. can you squeeze it in the same cell below Cage #? Do not change dimensions or anything else from the code: 
Worked for 1m 54s

Fixed the script so it:

Reads the Cage Barcode spreadsheet column.
Stores the value with each cage.
Prints Cage Barcode: [value] directly below Cage #: [value] in the same header cell.
Keeps all card, row, column, and page dimensions unchanged.
Uses the original font size whenever no barcode is present.

The updated code passed syntax validation and successfully generated a test PDF.

Pasted%20code%281%29_with_cage_barcode.py
