...
def build_notecards_pdf_bytes(
    ...
    for page_idx in range(front_pages):
        ...
        # Back page: keep each back in the same physical slot as its front.
        # Stock cages intentionally draw nothing in their slot, leaving that back
        # side blank for duplex printing.
        for slot_idx, cage in enumerate(group):
            slot_x, slot_y = CARD_SLOTS[slot_idx]
