...
def safe_bool(value: object, default: bool = True) -> bool:
    ...
    if not text:
        return default
    if text in {"true", "t", "yes", "y", "1", "x", "checked", "include", "print"}:
        return True
    if text in {"false", "f", "no", "n", "0", "unchecked", "exclude", "skip", "dont print", "don't print"}:
        return False
...
def normalize_settings_local(settings: dict[str, object] | None) -> dict[str, str]:
    normalized = dict(DEFAULT_SETTINGS)
    if settings:
        normalized.update({key: safe_str(value) for key, value in settings.items()})
...
def build_notecards_pdf_bytes_lazy(*args, **kwargs):
    """
    Import notecard only when a PDF is actually generated. This keeps the Shiny
    app startup-safe on Posit even if a PDF dependency or notecard import fails.
    The error will show in the Status panel instead of crashing app startup.
    """
    notecard = importlib.import_module("notecard")
    return notecard.build_notecards_pdf_bytes(*args, **kwargs)
...
app_ui = ui.page_sidebar(
    ...
    ui.p(
        ...
        "Cards are sorted with mating cages first, then stock cages."
    ),
    ui.h4("Status"),
...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def settings() -> dict[str, str]:
        return normalize_settings_local(
            {
    ...
    def generation_result() -> dict[str, object] | None:
        ...
        try:
            ...
            content, metadata = build_notecards_pdf_bytes_lazy(
                xlsx_source=df,
    ...
    def status() -> str:
        ...
        lines = [
            ...
            f"Layout: {metadata.get('cards_per_front_page', 4)} cards per front page; mating cages are printed first, then stock cages.",
            ...
            "Back pages are intentionally shown upside down in the PDF preview so landscape long-edge duplex prints upright behind the correct front card.",
